...
* **Objects:** prior p(theta), likelihood p(D|theta), posterior p(theta|D) ∝ p(D|theta) p(theta), predictive p(y*|x*,D) = ∫ p(y*|x*,theta) p(theta|D) dtheta dtheta.
...
Would you like me to also make a **step-by-step side-by-side math table** of SGD vs Langevin vs SGLD vs SGHMC (updates, assumptions, outcomes)? That’s often a very clean way to see why the ports are more complex than the blog suggests.
